...
- Dynamic messaging MUST follow WCAG live region guidance:
  - **Form-level blocking errors** (e.g., submission failed, multiple fields invalid): Use `role="alert"` with `aria-live="assertive"` and `aria-atomic="true"` in a container that is present in the DOM on load. This is for critical messages that require immediate attention. Reference: [WCAG ARIA19](https://www.w3.org/WAI/WCAG22/Techniques/aria/ARIA19).
  - **Inline field-level validation errors**: Use `role="status"` with `aria-live="polite"` and `aria-atomic="true"`. These errors appear as users interact with individual fields. Using "assertive" would be too disruptive when filling out multiple fields. The primary accessibility pathway is `aria-invalid="true"` + `aria-describedby` on the input; the live region is supplementary. Reference: [WCAG ARIA21](https://www.w3.org/WAI/WCAG22/Techniques/aria/ARIA21).
  - **Warnings or success/status updates** that do not block submission SHOULD use `role="status"` (implicit `aria-live="polite"`) or an explicit `aria-live="polite"` region with `aria-atomic="true"`. Reference: [WCAG ARIA22](https://www.w3.org/WAI/WCAG22/Techniques/aria/ARIA22).
  - **Hints and helper text** that are static or shown on focus should be referenced with `aria-describedby` so they are announced when the control receives focus. If hints update dynamically, keep the hint container in the DOM and mark it as `aria-live="polite"`.
  - Always associate each message region with its control using `aria-describedby` so screen readers can move back to the control and still access the guidance.
