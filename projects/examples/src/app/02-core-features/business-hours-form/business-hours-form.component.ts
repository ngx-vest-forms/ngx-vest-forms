...
export class BusinessHoursFormComponent {
  ...
  private readonly viewModel = computed(() => {
    ...
    // Guard against undefined during the very first render cycle
    const value = this.formValue() ?? this.initialBusinessHoursFormData;
    const fallbackBusinessHours =
      this.initialBusinessHoursFormData.businessHours;

    return {
      formValue: () => value,
      formValid: formState?.valid ?? false,
      formErrors: formState?.errors ?? {},
      businessHoursModel: value?.businessHours ?? fallbackBusinessHours,
    };
  });
