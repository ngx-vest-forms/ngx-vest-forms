...
@Component({
  ...
  template: `
    ...
    >
      <div class="relative">
        <!-- Step connector lines background -->
        <div
          class="absolute top-4 left-0 -z-10 flex w-full -translate-y-1/2 items-center"
          aria-hidden="true"
        >
          @for (
            step of steps();
          ...
          @for (
            ...
            <li class="relative min-w-0">
              <div
                ...
                [class.items-center]="i > 0 && !last"
                [class.items-end]="last"
              >
                <!-- Step circle with screen reader text -->
                <span
                  class="flex h-8 w-8 items-center justify-center rounded-full text-sm font-medium transition-colors duration-300"
                  [class.bg-blue-600]="getStepStatus(step.id) === 'completed'"
