...
export const createPurchaseValidationSuite = (
  ...
  return create((model: PurchaseFormModel) => {
    ...
    // Vest 6: memo() is supported with suite.only().run().
    // It avoids re-running expensive async checks when dependencies are unchanged.
    memo(() => {
