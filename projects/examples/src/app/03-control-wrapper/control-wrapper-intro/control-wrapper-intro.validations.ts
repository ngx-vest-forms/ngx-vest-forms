import type { NgxVestSuite } from 'ngx-vest-forms/core';
 ...
 */
export const controlWrapperIntroValidationSuite: NgxVestSuite<ControlWrapperIntroFormModel> =
  staticSuite(
    (data: Partial<ControlWrapperIntroFormModel> = {}, field?: string) => {
      ...
      test('email', 'Consider using a professional email provider', () => {
        ...
        if (data.email && data.email.includes('@')) {
          ...
          // Show warning if domain IS in casual providers list
          if (casualProviders.includes(domain)) {
            throw new Error('validation failed'); // Trigger the warning message
          }
        }
      });
