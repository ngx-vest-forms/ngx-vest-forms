...
/**
 * Recursively converts specified object properties (by key) from object-with-numeric-keys to arrays.
  ...
  if (object && typeof object === 'object') {
    if (isNumericObject(object)) {
      ...
      for (const [key, value] of Object.entries(object)) {
        ...
        // Check if this value was changed during processing
        if (processedValue !== value) {
          hasChanges = true;
        }
      }
      // If parent cascade is true (parent was explicitly converted), we also convert this numeric object.
