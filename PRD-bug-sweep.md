...
- **Bundle D — equality (unit, no TestBed):** two distinct cyclic objects with same structure compare equal; self-referential objects compare equal to themselves; deep trees (more than 10 levels) compare correctly without depth fallback; `Map`/`Set`/`Date`/`RegExp` comparisons match a documented contract (proposal: by reference for `Map`/`Set` unless we deep-compare entries — pick reference-only for now and document).
